...
- [ ] Remove `npm run test:run` from `.husky/pre-commit`
- [ ] Remove redundant `docker pull` from `deploy-vps.yml`

### This Week

- [ ] Add Docker layer caching to `docker.yml`
- [ ] Consolidate CI jobs or add artifact sharing
- [ ] Create `eslint.config.fast.js` for pre-commit

### This Month

- [ ] Evaluate happy-dom migration
- [ ] Plan husky v10 migration
- [ ] Review Claude command efficiency
