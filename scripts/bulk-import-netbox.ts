...
const VENDORS: VendorConfig[] = [
	...
	// Servers - High Priority
	{
		...
		filter: (d) =>
			d.u_height >= 1 && (d.model.includes('PowerEdge') || d.model.includes('PowerVault'))
	},
	{
		name: 'HPE',
		defaultCategory: 'server',
		filter: (d) =>
			d.u_height >= 1 &&
			(d.model.includes('ProLiant') || d.model.includes('Aruba') || d.model.includes('Apollo'))
	},
	{
		name: 'Supermicro',
		defaultCategory: 'server',
...
async function fetchJson<T>(url: string): Promise<T> {
	...
	const response = await fetch(url, {
		headers: {
			...
			'User-Agent': 'Rackarr-Bulk-Import'
		}
	});
...
function inferCategory(device: NetBoxDevice, vendorConfig: VendorConfig): DeviceCategory {
	const model = device.model.toLowerCase();
	const slug = device.slug.toLowerCase();

	// Check vendor-specific overrides first
	if (vendorConfig.categoryOverrides) {
		for (const [pattern, category] of Object.entries(vendorConfig.categoryOverrides)) {
			if (model.includes(pattern.toLowerCase()) || slug.includes(pattern.toLowerCase())) {
				return category;
			}
		}
	}
	...
	if (model.includes('nas') || model.includes('diskstation') || model.includes('rackstation'))
		return 'storage';
	if (model.includes('powervault') || model.includes('storage')) return 'storage';
	if (model.includes('kvm') || model.includes('console')) return 'kvm';
	if (model.includes('atem') || model.includes('receiver') || model.includes('amplifier'))
		return 'av-media';
...
async function downloadImage(url: string, destPath: string): Promise<boolean> {
	try {
		const response = await fetch(url, {
			headers: { 'User-Agent': 'Rackarr-Bulk-Import' }
		});
...
function getGenericDevices(): ImportedDevice[] {
	return [
		// Servers - generic
		{ slug: '1u-server', model: 'Server', u_height: 1, is_full_depth: true, category: 'server' },
		{ slug: '2u-server', model: 'Server', u_height: 2, is_full_depth: true, category: 'server' },
		{ slug: '4u-server', model: 'Server', u_height: 4, is_full_depth: true, category: 'server' },
		...
		// Power - generic
		{ slug: '1u-pdu', model: 'PDU', u_height: 1, is_full_depth: false, category: 'power' },
		{ slug: '2u-ups', model: 'UPS', u_height: 2, is_full_depth: true, category: 'power' },
		{ slug: '4u-ups', model: 'UPS', u_height: 4, is_full_depth: true, category: 'power' },

		// KVM
		{ slug: '1u-kvm', model: 'KVM', u_height: 1, is_full_depth: true, category: 'kvm' },
		{
		...
		// Blanks
		{ slug: '0-5u-blank', model: 'Blank', u_height: 0.5, is_full_depth: false, category: 'blank' },
		{ slug: '1u-blank', model: 'Blank', u_height: 1, is_full_depth: false, category: 'blank' },
		{ slug: '2u-blank', model: 'Blank', u_height: 2, is_full_depth: false, category: 'blank' },
		{ slug: '3u-blank', model: 'Blank', u_height: 3, is_full_depth: false, category: 'blank' },
		{ slug: '4u-blank', model: 'Blank', u_height: 4, is_full_depth: false, category: 'blank' },

		// Shelves
		{ slug: '1u-shelf', model: 'Shelf', u_height: 1, is_full_depth: true, category: 'shelf' },
		{ slug: '2u-shelf', model: 'Shelf', u_height: 2, is_full_depth: true, category: 'shelf' },
		{
...
async function main(): Promise<void> {
	...
	for (const vendorConfig of VENDORS) {
		try {
			const { devices, stats } = await importVendorDevices(vendorConfig, dryRun || statsOnly);
