...
const GROUPING_STORAGE_KEY = 'rackarr-device-grouping';
...
export function loadGroupingModeFromStorage(): DeviceGroupingMode {
	try {
		...
		console.warn('[rackarr] Failed to load grouping mode from localStorage:', e);
	}
	return 'brand';
}
...
export function saveGroupingModeToStorage(mode: DeviceGroupingMode): void {
	try {
		...
		console.warn('[rackarr] Failed to save grouping mode to localStorage:', e);
	}
}
