...
function orderDeviceTypeFields(dt: DeviceType): Record<string, unknown> {
	...
	// --- Component Arrays ---
	if (dt.interfaces !== undefined && dt.interfaces.length > 0) ordered.interfaces = dt.interfaces;
	...
	// --- Subdevice Support ---
	if (dt.subdevice_role !== undefined) ordered.subdevice_role = dt.subdevice_role;
...
function orderPlacedDeviceFields(device: PlacedDevice): Record<string, unknown> {
	...
	if (device.custom_fields !== undefined) ordered.custom_fields = device.custom_fields;
