...
describe('Share Utilities', () => {
	...
	describe('toMinimalLayout', () => {
		...
		it('only includes device types that are used', () => {
			...
			layout.rack.devices = [
				{ id: 'device-1', device_type: 'used-server', position: 5, face: 'front' }
			];
		...
		it('abbreviates category names', () => {
			...
			layout.rack.devices = [
				{ id: 'device-1', device_type: 'test-server', position: 1, face: 'front' }
			];
		...
		it('omits optional model when not set', () => {
			...
			layout.rack.devices = [
				{ id: 'device-1', device_type: 'minimal-device', position: 1, face: 'front' }
			];
		...
		it('includes model when set', () => {
			...
			layout.rack.devices = [
				{ id: 'device-1', device_type: 'full-device', position: 1, face: 'front' }
			];
	...
	describe('fromMinimalLayout', () => {
		...
		it('generates new IDs for devices', () => {
			...
			layout.rack.devices = [
				{ id: 'original-id', device_type: 'test-device', position: 5, face: 'front' }
			];
		...
		it('generates unique UUIDs for each device', () => {
			...
			layout.rack.devices = [
				...
				{ id: 'device-2', device_type: 'test-device', position: 2, face: 'front' },
				{ id: 'device-3', device_type: 'test-device', position: 3, face: 'front' }
			];
		...
		it('expands abbreviated categories back to full names', () => {
			...
			layout.rack.devices = [{ id: 'd1', device_type: 'test-net', position: 1, face: 'front' }];
	...
	describe('encodeLayout', () => {
		...
		it('produces reasonably sized output', () => {
			const layout = createTestLayout();
			layout.device_types = [
				{ slug: 'server-1', u_height: 2, colour: '#3b82f6', category: 'server' }
			];
			layout.rack.devices = [
		...
		it('handles layout with many devices', () => {
			const layout = createTestLayout();
			layout.device_types = [{ slug: 'server', u_height: 1, colour: '#000', category: 'server' }];
	...
	describe('round-trip encode/decode', () => {
		...
		it('preserves device types with core properties', () => {
			...
			original.rack.devices = [
				{ id: 'device-1', device_type: 'dell-r740', position: 10, face: 'front' }
			];
		...
		it('preserves manufacturer field when set', () => {
			...
			original.rack.devices = [
				{ id: 'device-1', device_type: 'cisco-switch', position: 1, face: 'front' }
			];
		...
		it('omits manufacturer when not set', () => {
			...
			original.rack.devices = [
				{ id: 'device-1', device_type: 'generic-device', position: 1, face: 'front' }
			];
		...
		it('preserves placed devices', () => {
			const original = createTestLayout();
			original.device_types = [
				{ slug: 'test-device', u_height: 2, colour: '#000', category: 'server' }
			];
			original.rack.devices = [
				{ id: 'device-1', device_type: 'test-device', position: 5, face: 'front' },
				{ id: 'device-2', device_type: 'test-device', position: 10, face: 'rear' }
			];
		...
		it('only includes used device types after round-trip', () => {
			...
			original.rack.devices = [{ id: 'd1', device_type: 'used', position: 1, face: 'front' }];
	...
	describe('generateShareUrl', () => {
		...
		beforeEach(() => {
			Object.defineProperty(window, 'location', {
				value: {
					...originalLocation,
					origin: 'https://app.rackarr.com',
					pathname: '/',
					href: 'https://app.rackarr.com/'
				},
				writable: true
			});
		});
		...
		it('generates URL with l parameter', () => {
			...
			expect(url).toContain('https://app.rackarr.com/');
